    ...
    def find_max_steering_angle(self) -> Tuple[float, float]:
        ...
        for angle_deg in range(1, 50):
            ...
            if sol_pos.valid and sol_neg.valid:
                ...
                pole_angle = math.degrees(math.atan(L / R))
                max_pole = pole_angle
            else:
                break
        return max_inner, max_pole
...
class AckermannSteeringModel:
    ...
    def calculate_ackermann_geometry(self, pole_angle_deg: float) -> SteeringResult:
        ...
        if not four_bar_sol.valid:
            max_inner_rad, max_pole = self.four_bar.find_max_steering_angle()
            safe_pole = max_pole * 0.95 * direction
            safe_inner = math.radians(max_inner_rad * 0.95)
        ...
        inner_angle = four_bar_sol.inner_angle
        outer_angle = four_bar_sol.outer_angle
        ...
        return SteeringResult(
            ...
            transmission_angle_inner=four_bar_sol.transmission_angle_inner,
            transmission_angle_outer=four_bar_sol.transmission_angle_outer,
            linkage_interference=four_bar_sol.interference,
